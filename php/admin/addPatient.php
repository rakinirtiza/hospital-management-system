    ...
    <div class="db-connect">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (empty($fnameErr) && empty($lnameErr) && empty($phoneErr) && empty($emailErr) && empty($dobErr) && empty($genderErr) && empty($addressErr) && empty($diseaseErr) && empty($doctorErr) && empty($admissionErr) && empty($roomErr) && empty($departmentErr)) {
                ...
                $stmt->bind_param("ssssssssiiss", $fname, $lname, $phone, $email, $dob, $gender, $address, $disease, $department_id, $doctor, $admission, $room);

                if ($stmt->execute()) {
                    echo "<script>alert('Patient successfully added!');</script>";
                    $fname = $lname = $phone = $email = $dob = $gender = $address = $disease = $department_id = $doctor = $admission = $room = "";
                } else {
    ...
    <main class="main-section">
        ...
        <section class="patient-list-section">
            ...
            <table class="patient-table">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT p.*, d.doctor_fname, d.doctor_lname, dep.department_name FROM patients_info p JOIN doctors_info d ON p.doctor_id = d.doctor_id JOIN departments_info dep ON p.department_id = dep.department_id ORDER BY p.patient_id ASC";
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <td>
                                <a href='update_patient.php?id={$row['patient_id']}' class='edit-btn'>Update</a>
